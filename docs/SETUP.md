...
管轄裁判所も神戸地方裁判所伊丹支部で記入済みです。

**ただし、次はまだ未対応です。配布先を広げる（学校の生徒全体、一般公開など）前に済ませてください。**

1. **公開 URL を用意する。** App Store Connect はプライバシーポリシーの URL を必須で求めます。
...
2. App Store Connect → App のページ → 「App のプライバシー」で、収集するデータを申告する。
   本アプリの実態に沿った回答は次のとおりです。

   | データ種別 | 収集するか | 用途 | トラッキングに使うか | 利用者に関連付けられるか |
   |---|---|---|---|---|
   | 連絡先情報（メール・電話番号） | 収集しない | — | — | — |
   | 名前（表示名） | **収集する** | App の機能 | いいえ | **はい**（アカウントに紐づくため） |
   | ユーザー ID（ハンドル・iCloud 識別子） | **収集する** | App の機能 | いいえ | **はい**（アカウントそのものの識別子のため） |
   | 写真またはビデオ | **収集する** | App の機能 | いいえ | **はい**（暗号化され内容は読めないが、送信者アカウントには紐づくため） |
   | その他のユーザーコンテンツ（メッセージ） | **収集する** | App の機能 | いいえ | **はい**（同上） |
   | 位置情報 / 購入履歴 / 検索履歴 / 使用状況データ / 診断 / 広告データ | 収集しない | — | — | — |

   > 「トラッキングに使うか」と「利用者に関連付けられるか」は別々の質問です。
   > 本アプリは広告 SDK・解析 SDK の類を一切組み込んでいないため、
   > **トラッキングは全項目「いいえ」**（アプリ全体としても「トラッキングを行わない」と回答する）。
   > 一方で、暗号化されて開発者が中身を読めないデータでも、
   > 「どのアカウントのものか」をシステムが管理している以上は
   > **「利用者に関連付けられるか」は「はい」**になる。この2つを混同しないこと。

3. 「App の使用許諾契約（EULA）」に利用規約を貼るか、URL を指定する。指定しない場合は
