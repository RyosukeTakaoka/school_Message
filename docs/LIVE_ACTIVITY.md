...
> **いまは見送っています。**
> 検討の結果、**Bluetooth の成功率が 1% も上がらない**（Live Activity は
> バックグラウンド実行を与えない）ため、Xcode でのターゲット追加に見合わないと判断しました。
> 効果は「動いているのが見えていれば、アプリを強制終了されにくくなる」という
> 見かけ上のものだけです。
>
> アプリ側のコードは残してあるので、やりたくなったらこの手順どおりに進めれば動きます。
> `docs/TODAY.md` の当日手順からは外してあります。
...
Live Activity が iPad で使えるかは iPadOS の版によって違います。
使えない端末では、この作業をしてもロック画面には何も出ません
（すれ違い通信そのものは変わらず動きます）。

いまは診断欄を出していません。作業を再開するときは、
`StreetPassView.swift` の「動作状況」セクションにある
`if streetPass.isLiveActivityRunning` の条件を外すと、
`streetPass.isLiveActivityAvailable` / `streetPass.liveActivityError` を
そのまま画面に出せます（`StreetPassStore` に用意してあります）。
